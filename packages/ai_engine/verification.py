...
def generate_verification_question(
    ...
    **kwargs
    ...
    """
    misc = diagnosed_misconception if diagnosed_misconception is not None else kwargs.get("resolved_misconception")
    concept = concept_context if concept_context is not None else (kwargs.get("concept") or kwargs.get("active_concept"))

    if not isinstance(misc, str) or not misc.strip():
    ...
    diagnosed_misconception = misc.strip()
    concept_context = concept.strip()
