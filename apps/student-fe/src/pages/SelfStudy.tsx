  ...
  return (
    <div className="min-h-screen py-10 px-4 max-w-5xl mx-auto">
      <div className="mb-8">
        ...
        <p className="adhd-hide text-neutral-500 dark:text-neutral-400">
      ...
      <div className="bg-white dark:bg-[#1E1E24] p-8 rounded-3xl border border-black/5 dark:border-white/10 shadow-sm mb-8 text-center transition-all">
        <Upload className="w-12 h-12 mx-auto text-brand mb-4" />
        <h2 className="text-xl font-bold text-neutral-900 dark:text-white mb-4">Upload Your PDF Notes</h2>
        <input 
          type="file" 
          accept=".pdf" 
          onChange={handleFileChange}
          className="block w-full max-w-sm mx-auto text-sm text-neutral-500 file:mr-4 file:py-2.5 file:px-5 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-brand/10 file:text-brand hover:file:bg-brand/20 transition cursor-pointer mb-6"
        />
        <button 
          onClick={handleUpload}
          disabled={!file || isUploading}
          className="px-8 py-3 rounded-full bg-brand text-white text-sm font-semibold disabled:opacity-50 hover:bg-brand-strong shadow-lg shadow-brand/20 transition flex items-center justify-center mx-auto space-x-2"
        >
          {isUploading ? (
            <>
      ...
      {pdfUrl && (
        <div className="mt-8 bg-white dark:bg-[#1E1E24] p-8 rounded-3xl border border-black/5 dark:border-white/10 shadow-sm animate-in slide-in-from-bottom-6 fade-in duration-500">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
            <div>
              <h3 className="text-xl font-bold text-neutral-900 dark:text-white flex items-center">
                <span className="bg-gradient-to-r from-brand to-[#EC4899] text-transparent bg-clip-text mr-2">LearnLens</span> 
                Smart Summary
              </h3>
              <p className="adhd-hide text-sm text-neutral-500 mt-1">Struggling with walls of text? Let AI break it down for you.</p>
            </div>
            {!aiSummary && (
              <button 
                onClick={handleGenerateSummary}
                disabled={isSummarizing}
                className="px-5 py-2.5 rounded-full bg-gradient-to-r from-brand to-[#EC4899] text-white text-sm font-bold shadow-md hover:shadow-lg disabled:opacity-50 transition flex items-center whitespace-nowrap"
              >
                {isSummarizing ? (
                  <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Processing Text...</>
                ) : (
                  <><Sparkles className="w-4 h-4 mr-2" aria-hidden="true" /> Generate ADHD/Dyslexia Summary</>
                )}
              </button>
            )}
          </div>
          ...
          {aiSummary && (
            <div className="prose prose-sm sm:prose-base dark:prose-invert max-w-none bg-brand/5 dark:bg-brand/10 p-6 rounded-2xl border border-brand/20">
              {aiSummary.split('\\n').map((line, i) => (
                <p key={i} className="mb-2 leading-relaxed" dangerouslySetInnerHTML={{ __html: line
                  .replace(/### (.*)/g, '<h4 class="text-lg font-bold text-brand mb-3">$1</h4>')
